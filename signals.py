...
async def cmd_post(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "Можешь написать просто мысль, наблюдение, анализ — "
        "бот оформит в стиле White Wolf.",
        parse_mode="Markdown"
    )
    await state.set_state(PostForm.entering_text)


@router.message(PostForm.entering_text, F.from_user.id == config.ADMIN_ID)
async def handle_post_text(message: Message, state: FSMContext):
    raw_text = message.text

    # Отправляем на AI оформление
    await message.answer("🤖 Оформляю пост...")
    formatted = await _format_custom_post(raw_text)
    ...
    await message.answer(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [
                ...
                InlineKeyboardButton(text="✏️ Переписать",   callback_data="post_rewrite"),
            ],
            [InlineKeyboardButton(text="❌ Отмена", callback_data="post_cancel")],
        ])
    )
    await state.set_state(PostForm.entering_text)
    await state.update_data(raw_text=raw_text, formatted_post=formatted)


@router.callback_query(F.data == "post_confirm")
async def cb_post_confirm(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    post = data.get("formatted_post", "")
    if not post:
        await call.answer("Нет текста для публикации")
        return
    await call.message.bot.send_message(
        chat_id=config.CHANNEL_ID,
        text=post,
...
@router.callback_query(F.data == "post_rewrite")
async def cb_post_rewrite(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
...
async def _format_custom_post(raw_text: str) -> str:
    """AI оформляет текст в стиле White Wolf"""
    try:
        prompt = (
            f"Оформи этот текст как пост для Telegram канала White Wolf — анонимный форекс трейдер.\n\n"
            f"Исходный текст:\n{raw_text}\n\n"
            f"Правила оформления:\n"
            f"- Сохрани смысл и факты полностью\n"
            f"- Стиль: уверенный, лаконичный, загадочный\n"
            f"- Добавь подходящие эмодзи\n"
            f"- Форматирование Telegram: **жирный**, _курсив_\n"
            f"- В конце: 🐺 и хэштеги #whitewolf\n"
            f"- Максимум 200 слов"
        )
        ...
        # Возвращаем базовое оформление без AI
        return (
            f"📝 *WHITE WOLF*\n\n"
            f"{raw_text}\n\n"
            f"🐺\n#whitewolf"
